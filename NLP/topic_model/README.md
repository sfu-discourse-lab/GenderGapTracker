...
### Output
The production script `lda_to_mongo.py` directly writes the results in the form of JSON objects to a new collection `topicModel` on MongoDB. This avoids having to maintain messy, external files containing results and allows us to continuously log each month's results in an organized manner. 
...
```sh
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp_2.11:2.4.5 lda_to_mongo.py
```
...
Alternately, for experimentation on large data sizes or for testing new functionality, a standalone topic model script (`lda_to_json.py`) is provided, that runs locally or on a Spark cluster. Unlike the production version, this process is designed to be customizable, both in terms of data content/size, as well as the individual functions implemented. In our case, we ran all our experiments using the extensive resources of Compute Canada. The below sections pertain only to the files `preproc.py` and `lda_to_json.py`.
...
```sh
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp_2.11:2.4.0 --driver-memory 16G --executor-memory 16G --num-executors 16 --executor-cores 4 lda_to_json.py --topics 15 --iter 100 --vocab 5000 --minDF 0.02 --maxDF 0.8 --partitions 100 --start 2019-11-01 --end 2020-01-31
```
...
```
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp_2.11:2.4.0 lda_to_json.py --help
...
```sh
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp_2.11:2.4.0 --driver-memory 16G --executor-memory 16G --num-executors 16 --executor-cores 4 lda_to_json.py --start 2020-01-01 --end 2020-01-31
```
...
```sh
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp_2.11:2.4.0 --driver-memory 16G --executor-memory 16G --num-executors 16 --executor-cores 4 lda_to_json.py --start 2020-01-01 --end 2020-01-31 --disable_outlet_log --disable_gender_log
```
