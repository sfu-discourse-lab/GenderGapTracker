...
gender_logger = utils.create_logger(
    "gender_recognition",
    log_dir="logs",
...
def get_gender(name, disable_cache=False, disable_service=False):
    ...
    # (len(name.split(" ")) > 6 is added to not resolve gender for long texts that are potentially nonsense
    # Length of 6 is chosen because Arabic names are commonly 6 words long.
    if (len(name.split(" ")) <= 1) or (len(name.split(" ")) > 6):
    ...
    if not disable_service:
        gender_logger.info(
            'Cache missed gender for "{0}". Calling services to identify gender.'.format(
                name
            )
        )
        ...
        if (
            ...
            if existing_item is None:
                ...
                if (
                    existing_item["gender"] is None
                    or existing_item["gender"] == "unknown"
                ):
                    firstname_cache_col.update(
                        {"_id": ObjectId(curr_name_id)}, {"$set": {"gender": gender}}
                    )
                    gender_logger.info(
                        'Update Cache "{0}" as first name for "{1}" as "{2}"'.format(
                            first_name, name, gender
                        )
                    )
                elif existing_item["gender"] != gender:
                    gender_logger.warning(
                        'Gender Mismatch for "{0}": FirstName Cache: "{1}"   {2}: "{3}"'.format(
                            first_name, existing_item["gender"], service_name, gender
                        )
                    )
        # Return result
...
def get_gender_from_cache(full_name):
    ...
    log_stmt_format = (
        '"{0}" cache result for First Name: "{1}" | Full Name: "{2}"\t: "{3}"'
    )
    ignore_list = []
    ...
    if existing_gender is None:
        gender_logger.debug(
            log_stmt_format.format(
                service_name, first_name, full_name, "Name Not Found"
            )
        )
        ...
        gender_logger.debug(
            log_stmt_format.format(service_name, first_name, full_name, gender)
        )
        return gender, service_name, ignore_list
    else:
        gender_logger.warning(
            log_stmt_format.format(service_name, first_name, full_name, "unknown")
        )
    ...
    if first_name is not None:  # only check for valid first names
        ...
        gender_logger.warning(
            'Can not extract first name from "{0}". Skipping First Name Services.'.format(
                full_name
            )
        )
...
def get_gender_from_service(full_name, ignore_list=[]):
    ...
    if (
        GENDERIZE_ENABLED
        and (first_name is not None)
        and (service_name not in ignore_list)
    ):
...
def get_viaf_gender(name, maximumRecords=1000):
    ...
    # DEPRECATED: Get gender from VIAF service
    try:
        some_url = (
...
def get_genderize_gender(full_name):
    ...
    if first_name is None:
        ...
        try:
            ...
            gender_return = session.get(
                "https://api.genderize.io/?", params=gender_payload
            )
...
def get_genderapi_gender(full_name):
    try:
        ...
        for field in ["input", "details"]:
            cache_obj.pop(field, None)
        print(
            'GenderAPI service call result for "{0}": "{1}"'.format(full_name, gender)
        )
        # Handle unknowns
        if gender is None:
            gender = "unknown"
            cache_obj["gender"] = "unknown"
        # Update cache -- 'q' attribute in cache_obj contains the same name as full name
